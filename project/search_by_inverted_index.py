...
def preprocess_files(mystem, file, files_list):
    ...
    document_length[int(files_list[files_list == file].index[0])] = text_length
...
def get_inverted_index(mystem, files_list, save=True):
    ...
    for file in tqdm_notebook(files_list):
        for word in preprocess_files(mystem, file, files_list):
            inverted_index[word].append(int(files_list[files_list == file].index[0]))
...
def get_search_result(query, inverted_index, files_list, document_length, num_res, return_sim=False):
    """
    ...
    if return_sim:
        result = [(doc, relevance_dict[doc]) for doc in sorted(relevance_dict, key=relevance_dict.get, reverse=True)[:num_res]]
    else:
